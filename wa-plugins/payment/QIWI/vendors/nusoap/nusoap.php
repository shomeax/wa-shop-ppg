...
class soap_transport_http extends nusoap_base {
	...
	function setEncoding($enc='gzip, deflate') {
		if (function_exists('gzdeflate')) {
			...
			if(version_compare('5.3', PHP_VERSION, '>') && function_exists('set_magic_quotes_runtime')) {
		...
		if ($rm == 'POST') {
			...
			if ($this->externalWSDLURL){
				if (strpos($this->externalWSDLURL, "http://") !== false) { // assume URL
    				...
    				if(function_exists('wa') && file_exists(wa()->getConfig()->getRootPath().'/wa-content/xml/wsdl-viewer.xsl')){
    					$chunk = fread($fp, 2048);
    					$pattern = '$1'."\r\n".'<?xml-stylesheet type="text/xsl" href="';
    					$pattern .=wa()->getUrl(true).'wa-content/xml/wsdl-viewer.xsl"?>'."\r\n";
    					$chunk = preg_replace('/(<\?xml[^\?]+\?>)/',$pattern,$chunk);
    					echo($chunk);
    				}
					fpassthru($fp);
				}
...
class wsdl extends nusoap_base {
	...
	function webDescription(){
		...
		if (!empty($_SERVER['REDIRECT_URL'])) {
			$PHP_SELF = preg_replace('/\bwsdl\b.*$/','',$_SERVER['REDIRECT_URL']);
